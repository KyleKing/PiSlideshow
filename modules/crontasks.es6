...
pyshell.on('close', (err) => {
  if (err)
    throw err;
  cronDebug('Finished and closed (sys_actions.py)');
});
...
function controlDisplay(schedule, sentText, task) {
  const JOB = new CronJob(schedule, () => {
    ...
    cronDebug(warn(`!! Completed ${task} Cron Task !!`));
  }, false);
  return JOB;
}
...
const weekDayAfternoonOn = controlDisplay('0 30 16 * * 1-5', 'tft true', 'WeekDayAfternoonOn');
...
// }, () => {
//   cronDebug(warn('!! Completed Fetch Cron Task !!'));
// }, false);
...
const FBI = new CronJob('0 0 * 12 * *', () => {
  ...
  cronDebug(warn('!! Completed Refresh FBI Task !!'));
}, false);
...
module.exports = {
  start() {
    ...
    weekDayAfternoonOn.start();
    weekEndMorningOn.start();
    // weekDayMorningOff.start();
    sleepDisplay.start();

    const keepAwake = new CronJob('0 0,15,30,45 * * * *', () => {
      cronDebug('...Keeping PiSlideshow Awake...');
    }, () => {
      cronDebug('Completed keepAwake cron task');
    }, false);
    keepAwake.start();
