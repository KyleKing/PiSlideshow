...
def stopwatch(message):
    ...
    try:
        ...
        t1 = time.time()
        config.send('Total elapsed time for %s: %.3f' % (message, t1 - t0))
...
class dbox_syncer(object):
    ...
    def __init__(self, token, local_dir, dbox_dir):
        ...
        config.send('Dropbox folder name:', self.dbox_dir)
        config.send('Local directory:', self.local_dir)
        if not os.path.exists(self.local_dir):
            config.send('Error:', self.local_dir, 'does not exist on your file system')
            sys.exit(1)
        elif not os.path.isdir(self.local_dir):
            config.send('Error:', self.local_dir, 'is not a folder on your file system')
            sys.exit(1)
        config.send('')
    ...
    def start_sync(self):
        ...
        for filename in local_fns:
            ...
            if filename.startswith('.'):
                ...
                if not (isinstance(md, dropbox.files.FileMetadata) and size == md.size):
                    config.send('Replacing:', filename)
                    os.unlink(fullname)
                    self.download(fullname)
                    new_imgs = True
            else:
                config.send('No corresponding file on Dropbox. Deleting:', fullname)
                os.unlink(fullname)
        config.send('')  # break
        ...
        for idx, db_fn in enumerate(listing.keys()):
            if db_fn not in local_fns:
                ...
                config.send('Downloading new file:', db_fn, 'to:', fullname)
                self.download(fullname)
                new_imgs = True
        config.send('')  # break
    ...
    def list_folder(self, folder, subfolder=''):
        ...
        try:
            ...
            config.send('Folder listing failed for:', path, '-- assumed empty:', err)
            return {}
        else:
            rv = {}
